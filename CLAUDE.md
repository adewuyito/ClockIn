...
- **Not yet verified:** funded devnet keypair (`GBZqhLZXAjBtfeVkVWMYWFN8DGmxskwKna3UEXGvfh8P` — public faucet has been rate-limited on every attempt so far, retry or use faucet.solana.com); devnet deploy (Phase 3); real on-device MWA round-trip (Phase 4) — the code exists and looks correct on review but hasn't been run against a live wallet app yet.
- **Caveat on phase-ordering:** the roadmap sequences MWA (Phase 4) and program integration (Phase 5) after devnet deploy (Phase 3, not yet done) — but the app-core commits (`c527fb3`, `7060f4a`, `2d23e8f`) already built the Phase 5/6-shaped code (service layer, screens) ahead of that. That's fine — the code is real and reviewed, not a mock — but Phase 3 (actual devnet deploy) and Phase 4 (a real on-device signed transaction) are still the two things standing between "the code exists" and "the core loop is proven end-to-end."
