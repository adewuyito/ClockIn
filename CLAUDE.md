...
- **Stitch source of truth for the Profile screen:** https://stitch.withgoogle.com/projects/2859884629275757623?node-id=3f63ce256a9340218f6de0acb00b233d — the app's `MyProfileScreen` should be refined to match this exactly. Not yet pulled into the codebase: the Stitch MCP connector currently fails with `Incompatible auth server: does not support dynamic client registration` on every call (`list_projects`, `list_design_systems`, etc.) — a connector-side auth config issue, not fixable by retrying; needs reconnecting/reconfiguring in Claude Code's MCP settings. The project URL itself is also behind Stitch login, so a plain fetch only returns the generic landing page, not the design. Until one of those is fixed, matching this screen needs the design shared directly (screenshots, exported tokens/CSS).
